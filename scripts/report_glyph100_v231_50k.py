#!/usr/bin/env python3
"""Build post-training report for Glyph-100M v2.3.1 45k->50k run."""
...
LOG_PATH = ROOT / "logs" / "glyph-100m-v2_3_1-50k" / "train.log"
CKPT_DIR = ROOT / "checkpoints" / "glyph-100m-v2_3_1-50k"
LATEST = CKPT_DIR / "latest.pt"
STEP_50K = CKPT_DIR / "step_0050000.pt"
REPORT_MD = ROOT / "reports" / "glyph100_v2_3_1_50k_diagnostic_report.md"
REPORT_JSON = ROOT / "reports" / "glyph100_v2_3_1_50k_diagnostic_report.json"
...
def main() -> None:
    ...
    if not LATEST.exists() or not STEP_50K.exists():
        raise SystemExit("50k checkpoint missing")
    ...
    step_meta = checkpoint_meta(STEP_50K)
    ...
    train_start_window = [row["loss"] for row in steps if 45000 < row["step"] <= 45500]
    train_mid_window = [row["loss"] for row in steps if 47500 <= row["step"] <= 48000]
    train_end_window = [row["loss"] for row in steps if 49500 <= row["step"] <= 50000]
    overfit_signal = (
        "no_obvious_overfit"
    ...
    expected_stage_tokens = (50000 - 45000) * TOKENS_PER_STEP

    payload = {
        "run": "glyph100_v2_3_1_45k_to_50k",
        "status": "complete" if last["step"] == 50000 and parsed["complete_line"] else "check_log",
        "log_path": str(LOG_PATH.relative_to(ROOT)),
        "checkpoint_dir": str(CKPT_DIR.relative_to(ROOT)),
        "latest_checkpoint": ckpt_meta,
        "step_50000_checkpoint": step_meta,
        ...
        "partial_files_preserved": {
            "50k_emergency_pt": (CKPT_DIR / "emergency.pt").exists(),
            "50k_partial_like_files": sorted(
                path.name
    ...
    lines = [
        ...
        f"- step checkpoint: `{payload['step_50000_checkpoint']['path']}`",
        "",
        "## Checkpoint metadata",
        "",
    ...
    lines.extend(
        [
            ...
            "",
            f"- 50k `emergency.pt`: `{payload['partial_files_preserved']['50k_emergency_pt']}`",
            f"- 50k partial-like files: `{payload['partial_files_preserved']['50k_partial_like_files']}`",
