#!/usr/bin/env python3
"""Evaluate Glyph-100M v2.4.2 checkpoints on source-specific validation bins."""
...
DOC_CANDIDATES = (
    ROOT / "data" / "processed" / "glyph100_v2_4_2_docs.jsonl",
    ROOT / "data" / "raw" / "glyph100_v2_4_2" / "glyph100_v2_4_2_docs.jsonl",
)
...
CHECKPOINTS = {
    "1k": ROOT / "checkpoints" / "glyph-100m-v2_4_2-5k" / "step_0001000.pt",
    "5k": ROOT / "checkpoints" / "glyph-100m-v2_4_2-5k" / "step_0005000.pt",
}
    ...
    try:
        with docs_path.open("r", encoding="utf-8") as source:
            for line_number, line in enumerate(source, 1):
                ...
                ids = processor.encode(record["text"], out_type=int)
                ids.append(eos_id)
...
def main() -> None:
    ...
    payload = {
        ...
        "notes": [
            ...
            "Each source is evaluated only on document-level validation records from v2.4.2.",
        ],
    }
    OUT_JSON.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    lines = [
        "# Glyph-100M v2.4.2 source-specific validation",
        "",
    ...
    lines.extend(["", "Negative deltas mean improvement at 5k.", ""])
