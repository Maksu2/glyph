#!/usr/bin/env python3
"""Compare Glyph-100M v2.3.1 35k vs 45k as base-LM continuations.
...
def compare_rows(row35: dict, row45: dict) -> dict:
    s35 = quality_score(row35)
    s45 = quality_score(row45)
    delta = s45 - s35
    if delta >= 2:
        winner = "45k"
    elif delta <= -2:
    ...
    for key, label in [
        ...
        a = row35["analysis"].get(key, 0)
        b = row45["analysis"].get(key, 0)
        if b < a:
    ...
    for bucket, label in [("wiki", "wiki residue"), ("web", "web residue"), ("pseudo_ency", "pseudo-ency")]:
        a = len(row35["residue_hits"][bucket])
        b = len(row45["residue_hits"][bucket])
        if b < a:
            improvements.append(f"mniej {label}")
        elif b > a:
            regressions.append(f"więcej {label}")
    if row45["analysis"].get("ends_naturally") and not row35["analysis"].get("ends_naturally"):
        improvements.append("naturalniejsze zakończenie")
    elif row35["analysis"].get("ends_naturally") and not row45["analysis"].get("ends_naturally"):
        regressions.append("gorsze zakończenie")

    return {
        "prompt_id": row45["prompt_id"],
        "category": row45["category"],
        "preset": row45["preset"],
        "score_35k": s35,
        "score_45k": s45,
        "delta": delta,
...
def render_samples_md(path: Path, rows45: list[dict], meta45: dict, tokenizer_hash: str) -> None:
    lines = [
        "# Glyph-100M v2.3.1 45k samples",
        "",
        "Base LM completion eval. To nie jest eval asystenta ani instruction-following.",
        "",
        f"- checkpoint: `{meta45['path']}`",
        f"- step: `{meta45['step']}`",
        f"- variant: `{meta45['variant']}`",
        f"- dataset: `{meta45['dataset_name']}`",
...
def render_comparison_md(path: Path, prompts: list[dict], rows35: list[dict], rows45: list[dict], comparisons: list[dict]) -> None:
    by35 = {(row["prompt_id"], row["preset"]): row for row in rows35}
    by45 = {(row["prompt_id"], row["preset"]): row for row in rows45}
...
def verdict_from_counts(counts: Counter, rows45: list[dict]) -> str:
    total = sum(counts.values())
    wins45 = counts.get("45k", 0)
    wins35 = counts.get("35k", 0)
    residue45 = sum(1 for row in rows45 if row["residue_hits"]["web"] or row["residue_hits"]["wiki"])
    repetition45 = sum(1 for row in rows45 if row["analysis"]["repeated_4grams"] > 0)
    cutoff45 = sum(1 for row in rows45 if not row["analysis"]["ends_naturally"])
    if wins45 >= int(total * 0.55) and wins45 >= wins35 * 2 and residue45 <= total * 0.1 and cutoff45 <= total * 0.75:
        return "A"
    if wins45 > wins35 and repetition45 <= total * 0.35:
        return "E"
    if wins35 > wins45:
        return "D"
    return "E"
...
def render_report(path: Path, payload: dict) -> None:
    ...
    verdict_text = {
        ...
        "B": "Zatrzymać pretraining na 45k.",
        "C": "Zrobić mały SFT eksperyment na kopii 45k.",
        "D": "Poprawić dataset przed dalszym treningiem.",
        "E": "Zrobić dodatkowy eval/inference tuning przed decyzją.",
    }[verdict]
    lines = [
        ...
        f"- 45k: `{payload['checkpoints']['45k']['path']}` · step `{payload['checkpoints']['45k']['step']}` · variant `{payload['checkpoints']['45k']['variant']}`",
        ...
        "",
        "```json",
        json.dumps(payload["summaries"]["45k"], ensure_ascii=False, indent=2),
        "```",
        "",
        "## Werdykt",
        "",
...
def main() -> None:
    ...
    parser.add_argument("--checkpoint-45k", default="checkpoints/glyph-100m-v2_3_1-45k/latest.pt")
    ...
    parser.add_argument("--out-json", default="eval/glyph-100m/v2_3_1_45k_samples.json")
    parser.add_argument("--out-md", default="eval/glyph-100m/v2_3_1_45k_samples.md")
    parser.add_argument("--compare-md", default="eval/glyph-100m/v2_3_1_35k_vs_45k.md")
    parser.add_argument("--report-md", default="reports/glyph100_v2_3_1_45k_eval_report.md")
    ...
    ckpt45 = ROOT / args.checkpoint_45k
    meta35 = load_checkpoint_metadata(ckpt35)
    meta45 = load_checkpoint_metadata(ckpt45)
    for label, meta, expected_step in [("35k", meta35, 35000), ("45k", meta45, 45000)]:
    ...
    rows_by_checkpoint = {}
    for label, checkpoint in [("35k", ckpt35), ("45k", ckpt45)]:
    ...
    rows45 = rows_by_checkpoint["45k"]
    by35 = {(row["prompt_id"], row["preset"]): row for row in rows35}
    comparisons = [compare_rows(by35[(row45["prompt_id"], row45["preset"])], row45) for row45 in rows45]
    counts = Counter(row["winner"] for row in comparisons)

    payload = {
        "checkpoints": {"35k": meta35, "45k": meta45},
        ...
        "presets": PRESETS,
        "summaries": {"35k": summarize_rows(rows35), "45k": summarize_rows(rows45)},
        "winner_counts": dict(counts),
        "verdict": verdict_from_counts(counts, rows45),
        "samples_35k": rows35,
        "samples_45k": rows45,
    ...
    render_samples_md(out_md, rows45, meta45, tokenizer_hash)
    render_comparison_md(compare_md, prompts, rows35, rows45, comparisons)
    render_report(report_md, payload)
    print(
        f"done: winners 45k={counts.get('45k', 0)} "
        f"35k={counts.get('35k', 0)} tie={counts.get('tie', 0)} "
