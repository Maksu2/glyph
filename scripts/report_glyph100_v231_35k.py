#!/usr/bin/env python3
"""Build post-training report for Glyph-100M v2.3.1 25k->35k run."""
...
LOG_PATH = ROOT / "logs" / "glyph-100m-v2_3_1-35k" / "train.log"
CKPT_DIR = ROOT / "checkpoints" / "glyph-100m-v2_3_1-35k"
LATEST = CKPT_DIR / "latest.pt"
STEP_35K = CKPT_DIR / "step_0035000.pt"
REPORT_MD = ROOT / "reports" / "glyph100_v2_3_1_35k_report.md"
REPORT_JSON = ROOT / "reports" / "glyph100_v2_3_1_35k_report.json"
...
def main() -> None:
    ...
    if not LATEST.exists() or not STEP_35K.exists():
        raise SystemExit("35k checkpoint missing")
    ...
    step_meta = checkpoint_meta(STEP_35K)
    ...
    train_start_window = [row["loss"] for row in steps if 25000 < row["step"] <= 25500]
    train_mid_window = [row["loss"] for row in steps if 26500 <= row["step"] <= 27000]
    train_end_window = [row["loss"] for row in steps if 34500 <= row["step"] <= 35000]
    overfit_signal = (
        "no_obvious_overfit"
    ...
    expected_stage_tokens = (35000 - 25000) * TOKENS_PER_STEP

    payload = {
        "run": "glyph100_v2_3_1_25k_to_35k",
        "status": "complete" if last["step"] == 35000 and parsed["complete_line"] else "check_log",
        "log_path": str(LOG_PATH.relative_to(ROOT)),
        "checkpoint_dir": str(CKPT_DIR.relative_to(ROOT)),
        "latest_checkpoint": ckpt_meta,
        "step_35000_checkpoint": step_meta,
        ...
        "partial_files_preserved": {
            "step_0026000.pt": (CKPT_DIR / "step_0026000.pt").exists(),
            "emergency.pt": (CKPT_DIR / "emergency.pt").exists(),
            "manual_stop_marker_archived": bool(list((ROOT / "logs" / "glyph-100m-v2_3_1-35k").glob("emergency_stop.manual_stop_for_batch8_bench*.json"))),
        },
    ...
    lines = [
        ...
        f"- step checkpoint: `{payload['step_35000_checkpoint']['path']}`",
        "",
        "## Checkpoint metadata",
        "",
        ...
        f"- active duration, excluding manual stop gap: `{payload['active_duration_human']}`",
    ...
    lines.extend(
        [
            ...
            f"- interrupted segment clean exit seen: `{parsed['clean_interrupt']}`",
            "- GPU temps: not logged in `train.log`",
            "",
            "## Preserved partial files",
            "",
            f"- `step_0026000.pt`: `{payload['partial_files_preserved']['step_0026000.pt']}`",
            f"- `emergency.pt`: `{payload['partial_files_preserved']['emergency.pt']}`",
            f"- archived manual stop marker: `{payload['partial_files_preserved']['manual_stop_marker_archived']}`",
            "",
            "## Checkpoints",
            "",
