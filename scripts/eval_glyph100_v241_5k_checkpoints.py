#!/usr/bin/env python3
"""Compare Glyph-100M v2.4.1 checkpoints from 1k through 5k."""
...
CHECKPOINTS = {
    "1k": "checkpoints/glyph-100m-v2_4_1-1k/step_0001000.pt",
    "2k": "checkpoints/glyph-100m-v2_4_1-5k/step_0002000.pt",
    "3k": "checkpoints/glyph-100m-v2_4_1-5k/step_0003000.pt",
    "4k": "checkpoints/glyph-100m-v2_4_1-5k/step_0004000.pt",
    "5k": "checkpoints/glyph-100m-v2_4_1-5k/step_0005000.pt",
}
PRESETS = {
    "conservative": {"temperature": 0.6, "top_k": 30, "max_new_tokens": 80},
    "normal": {"temperature": 0.8, "top_k": 40, "max_new_tokens": 80},
}
PSEUDO_PATTERNS = (
    "województw",
    "w gminie",
...
def checkpoint_contract(state: dict, expected_step: int) -> dict:
    contract = {
        "step": state.get("step"),
    ...
    contract["valid"] = all(
        (
            contract["step"] == expected_step,
            contract["current_step"] == expected_step,
            contract["variant"] == "glyph-100m",
            contract["dataset_name"] == "glyph100_dataset_v2_4_1_core",
...
def write_samples_md(path: Path, rows: list[dict]) -> None:
    lines = [
        "# Glyph-100M v2.4.1 checkpoint samples",
        "",
        "Base-LM continuation eval. This is not an assistant or instruction-following benchmark.",
        "",
    ]
    for row in rows:
        diag = row["diagnostic"]
        lines.extend(
            [
                ...
                "```",
                "",
                f"Score `{diag['quality_score']}` · natural `{diag['natural_ending']}` · cutoff `{diag['cutoff_like']}` · repetition `{diag['repetition']}` · pseudo-ency `{bool(diag['pseudo_ency_hits'])}`",
                "",
            ]
        )
...
def main() -> None:
    ...
    parser.add_argument("--prompts", default=f"eval/glyph-100m/{PREFIX}_prompts.jsonl")
    ...
    for checkpoint_index, (label, relative) in enumerate(CHECKPOINTS.items()):
        print(f"loading {label}: {relative}", flush=True)
        model, state = load_model(ROOT / relative, device)
        expected_step = int(label[:-1]) * 1000
        contracts[label] = checkpoint_contract(state, expected_step)
    ...
    pairwise_results = {
        "1k_vs_5k": pairwise(rows, "1k", "5k"),
        "4k_vs_5k": pairwise(rows, "4k", "5k"),
    }
    validation = validation_losses()
    best_label = max(CHECKPOINTS, key=lambda label: summaries[label]["overall"]["avg_quality_score"])
    payload = {
        "run_id": PREFIX,
        "scope": "base-LM continuation checkpoint trajectory; no training performed",
        "prompts": len(prompts),
        "presets": PRESETS,
        ...
        "methodology_note": "Quality score is diagnostic and heuristic; manual sample review remains required.",
        "samples": rows,
    }
    ...
    lines.extend(
        [
            ...
            "## Pairwise heuristic",
            "",
            f"- 1k vs 5k: `{pairwise_results['1k_vs_5k']}`",
            f"- 4k vs 5k: `{pairwise_results['4k_vs_5k']}`",
