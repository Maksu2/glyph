...
def locate_input_archive() -> Path:
    ...
    if len(matches) != 1:
        raise RuntimeError(f"Expected exactly one {INPUT_ARCHIVE}, found: {matches}")
    return matches[0]
...
def main() -> None:
    ...
    archive = locate_input_archive()
    extract_tar_safely(archive, WORK_ROOT)
    payload_validation = verify_payload(WORK_ROOT)
    gpu = gpu_report()

    checkpoint = WORK_ROOT / "checkpoints/glyph-100m-base44k.pt"
    tokenizer = WORK_ROOT / "data/processed/tokenizer.model"
    dataset = WORK_ROOT / "data/sft/glyph100_sft_smoke_v0_3.jsonl"
    train_jsonl = WORK_ROOT / "data/sft/processed/glyph100_sft_smoke_v0_3_train.jsonl"
    val_jsonl = WORK_ROOT / "data/sft/processed/glyph100_sft_smoke_v0_3_val.jsonl"
    reports_dir = OUTPUT_ROOT / "preflight-reports"

    run(
        [
            sys.executable,
            str(WORK_ROOT / "scripts/colab_preflight.py"),
            "--run-id",
            RUN_ID,
            "--checkpoint",
            str(checkpoint),
    ...
    run(
        [
            sys.executable,
            str(WORK_ROOT / "scripts/colab_run_sft.py"),
            "--run-id",
            RUN_ID,
            "--base-checkpoint",
            str(checkpoint),
